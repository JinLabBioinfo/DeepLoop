...
def get_chromosome_from_filename(filename):
    ...
    """
    chr_index = filename.index('chr')  # index of chromosome name
...
def save_samples(input_dir, target_dir, matrix_size, multi_input=False, dir_3=None, combined_dir=None, anchor_dir=None, name='sample', chr_index=0, locus=18000):
    """
    ...
    if (name == 'enhance' or name == 'val_enhance') and multi_input:
        matrix_1 = load_chr_ratio_matrix_from_sparse(input_dir + input_folder_1, os.listdir(input_dir + input_folder_1)[chr_index], anchor_dir)
        matrix_2 = load_chr_ratio_matrix_from_sparse(target_dir, os.listdir(target_dir)[chr_index], anchor_dir)
        matrix_3 = None
        combined_matrix = None
    else:
        if multi_input:
            matrix_1 = load_chr_ratio_matrix_from_sparse(input_dir + input_folder_1, os.listdir(input_dir + input_folder_1)[chr_index], anchor_dir)
            matrix_2 = load_chr_ratio_matrix_from_sparse(input_dir + input_folder_2, os.listdir(input_dir + input_folder_2)[chr_index], anchor_dir)
            matrix_3 = load_chr_ratio_matrix_from_sparse(input_dir + input_folder_3, os.listdir(input_dir + input_folder_3)[chr_index], anchor_dir)
            combined_matrix = load_chr_ratio_matrix_from_sparse(target_dir, os.listdir(target_dir)[chr_index], anchor_dir)
        else:
            matrix_1 = load_chr_ratio_matrix_from_sparse(input_dir, os.listdir(input_dir)[chr_index], anchor_dir)
            matrix_2 = load_chr_ratio_matrix_from_sparse(target_dir, os.listdir(target_dir)[chr_index], anchor_dir)
            if dir_3 is not None:
                matrix_3 = load_chr_ratio_matrix_from_sparse(dir_3, os.listdir(dir_3)[chr_index], anchor_dir)
            else:
                matrix_3 = None
            if combined_dir is not None:
                combined_matrix = load_chr_ratio_matrix_from_sparse(combined_dir, os.listdir(combined_dir)[chr_index], anchor_dir)
            else:
                combined_matrix = None
    i = locus
...
def load_chr_ratio_matrix_from_sparse(dir_name, file_name, anchor_dir, dummy=5, ignore_sparse=False, force_symmetry=False):
    """
    ...
    for input_file, target_file in zip(input_files, target_files):
        ...
        if multi_input:
            ...
            for input_folder in input_folders:
                input_folder += '/'
                input_matrix = load_chr_ratio_matrix_from_sparse(input_dir + input_folder, input_file, anchor_dir)
            ...
            input_matrix = load_chr_ratio_matrix_from_sparse(input_dir, input_file, anchor_dir)
            target_matrix = load_chr_ratio_matrix_from_sparse(target_dir, target_file, anchor_dir)
...
def get_matrices_from_loci(input_dir,
    ...
    for file_1, file_2 in zip(input_files, target_files):
        ...
        loci_indices = (anchor_list['start'] <= loci[chr_name]) & (loci[chr_name] <= anchor_list['end'])
...
def get_heatmap(matrix, color_scale):
    ...
    return m.to_rgba(matrix)


def save_images_to_video(output_name):
    """
    ...
    """
    image_folder = 'images'
    video_name = 'demos/training_visualizations/' + output_name + '.avi'
    ...
    try:
        ...
        pass
    try:
        os.mkdir('demos/training_visualizations')
    except FileExistsError:
        pass
