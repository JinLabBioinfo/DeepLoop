...
def generate_cell_2Mb_matrix(data_dir, anchor_file, limit_2Mb=128, resize_amount=0.1):
    for cell in sorted_nicely(os.listdir(data_dir)):
        print(cell)
        anchor_to_anchor = pd.read_csv(data_dir + cell, delimiter='\t',
                                       names=['a1', 'a2', 'obs', 'exp'],
                                       usecols=['a1', 'a2', 'obs', 'exp'])
        anchor_to_anchor['ratio'] = (anchor_to_anchor['obs'] + 5) / (anchor_to_anchor['exp'] + 5)
        anchor_list = pd.read_csv(anchor_file, sep='\t', names=['chr', 'start', 'end', 'anchor', 'length'],
                                  usecols=['chr', 'start', 'end', 'anchor'])  # read anchor list file
        anchor_dict = anchor_list_to_dict(anchor_list['anchor'].values)  # convert to anchor --> index dictionary
        ...
        matrix = scipy.sparse.csr_matrix((anchor_to_anchor['ratio'], (rows, cols)),
                                         shape=(len(anchor_list), len(anchor_list)))  # construct sparse CSR matrix

        n_tiles = 64

        rotated = np.zeros((limit_2Mb, int(matrix.shape[1] * resize_amount * math.sqrt(2))))
        for i in range(n_tiles):
            window_size = int(matrix.shape[1] / n_tiles)
            start = window_size * i
            end = window_size * (i + 1)
            if end > matrix.shape[1]:
                matrix = rotated.shape[1]
            matrix_1 = cv2.resize(matrix[start: end, start: end].A, (int(matrix.shape[1] / n_tiles * resize_amount / 2), int(matrix.shape[0] / n_tiles * resize_amount / 2)),
                                 interpolation=cv2.INTER_AREA)
            rotated_1 = scipy.ndimage.rotate(matrix_1, 45, mode='constant', reshape=True)
            center_1 = int(rotated_1.shape[0] / 2)
            rotated_1 = rotated_1[center_1 - limit_2Mb: center_1, ...]

            rotated_window_size = int(rotated.shape[1] / n_tiles / 2)
            rotated_start = rotated_window_size * i
            rotated_end = rotated_window_size * (i + 1)
            if rotated_end > rotated.shape[1]:
                rotated_end = rotated.shape[1]
            rotated[..., rotated_start: rotated_end] += rotated_1
        yield rotated


def generate_genome_2Mb_matrix(data_dir, anchor_dir=None, anchor_file=None, limit_2Mb=128):
    try:
        os.mkdir('rotated_matrices/')
    except FileExistsError:
        pass
    if anchor_file is not None:
        anchor_list = pd.read_csv(anchor_file, sep='\t',
                                  names=['chr', 'start', 'end', 'anchor', 'length'],
                                  usecols=['chr', 'start', 'end', 'anchor'])  # read anchor list file
    for chr_file in sorted_nicely(os.listdir(data_dir)):
        if 'anchor_2_anchor' in chr_file or 'denoised.anchor.to.anchor' in chr_file:
            print(chr_file)
            chr_name = get_chromosome_from_filename(chr_file)
            if anchor_file is not None:
                chr_anchor_list = anchor_list[anchor_list['chr'] == chr_name][[['start', 'end', 'anchor']]]
            else:
                chr_anchor_list = pd.read_csv(anchor_dir + '%s.bed' % chr_name, sep='\t', names=['chr', 'start', 'end', 'anchor'],
                                          usecols=['start', 'end', 'anchor'])  # read anchor list file
            matrix = load_chr_ratio_matrix_from_sparse(data_dir, chr_file, anchor_dir, anchor_list=chr_anchor_list)
            rotated = scipy.ndimage.rotate(matrix.A, 45, mode='constant', reshape=True)
            center = int(rotated.shape[0] / 2)
            rotated = rotated[center - limit_2Mb: center, ...]
            rotated = cv2.resize(rotated, (int(rotated.shape[1] / 2), int(rotated.shape[0] / 2)), interpolation=cv2.INTER_AREA)
            data_folder = data_dir[data_dir.rfind('/', 0, len(data_dir) - 1)]
            try:
                os.mkdir('rotated_matrices/%s' % data_folder)
            except FileExistsError as e:
                print(e)
                pass

            np.save('rotated_matrices/%s/%s.npy' % (data_folder, chr_name), rotated)
            plt.imshow(rotated[..., :1000])
            plt.show()
